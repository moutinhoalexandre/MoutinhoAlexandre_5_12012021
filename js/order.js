...
contact.innerHTML += `
    ...
    <p class="fs-5"> Votre commande passée le <span class="fw-bold">${date[0].date}</span> à <span class="fw-bold">${date[0].hours}</span> d'un montant total de <span class="fw-bold">${convertPrice(total)}</span> a été validée.</p>
...
for (product of basket) {
    let indexProduct = basket.indexOf(product);
    let productList = document.getElementById("productsBasket");
...
let clickHome = document.getElementById("accueil");
clickHome.addEventListener("click", (e) => {
    clearBasket()
})

let clickBasket = document.getElementById("pillOnStorage");
clickBasket.addEventListener("click", (e) => {
  clearBasket();
});